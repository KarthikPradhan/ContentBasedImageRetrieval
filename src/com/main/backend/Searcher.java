...
public class Searcher {
	...
	public ArrayList<String> searchImg(String searchImg) throws IOException {
		
		BufferedImage img = null;
		boolean passed = false;
		if (searchImg.length() > 0) {
			File f = new File(searchImg);
			if (f.exists()) {
				try {
					img = ImageIO.read(f);
					passed = true;
		 ...
		 */
		// ImageSearcher searcher = new GenericDocValuesImageSearcher(30,
		// CEDD.class, ir);
		...
		ArrayList<String> foundImage= new ArrayList<>();
		for (int i = 0; i < hits.length(); i++) {
			String fileName = ir.document(hits.documentID(i)).getValues(DocumentBuilder.FIELD_NAME_IDENTIFIER)[0];
			System.out.println(hits.score(i) + ": \t" + fileName);
			foundImage.add(fileName);
